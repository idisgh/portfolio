<template>
  <div>
    <!-- Hero -->
    <section data-hero class="min-h-screen flex items-center px-5 sm:px-16 py-20">
      <div class="max-w-2xl">
        <div class="reveal text-[13px] tracking-[0.2em] text-[#999] mb-6 font-normal">WORK</div>
        <h1 class="reveal font-display text-[clamp(32px,5vw,64px)] font-normal tracking-tight text-white mb-3">Projects</h1>
        <p class="reveal text-[15px] text-[#999]">직접 참여하고 개발한 프로젝트입니다.</p>
      </div>
    </section>

    <!-- 필터 -->
    <div class="flex gap-2 px-5 sm:px-16 pb-10 overflow-x-auto scrollbar-none">
      <button
        v-for="f in filters"
        :key="f"
        class="flex-shrink-0 text-xs text-[#999] bg-transparent border border-[#1a1a1a] rounded-full px-3.5 py-1 cursor-pointer transition-all duration-200 hover:text-white hover:border-[#333]"
    ...
    <!-- Projects -->
    <section class="px-5 sm:px-16 pb-32">
      <div
        v-for="project in filteredProjects"
        :key="project.id"
        class="reveal group py-7 sm:py-9 border-t border-[#111] transition-colors duration-200 hover:bg-white/[0.005]"
      >
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-8">
          ...
          <component
            ...
            v-if="project.thumb"
            class="project-thumb flex-shrink-0 w-full sm:w-[140px] h-48 sm:h-[90px] rounded-md overflow-hidden border border-[#1a1a1a] sm:transition-all sm:duration-[350ms] sm:ease-in-out sm:group-hover:w-[200px] sm:group-hover:h-[128px] sm:group-hover:border-[#333] sm:group-hover:shadow-[0_8px_32px_rgba(0,0,0,0.5)] sm:group-hover:[transform:rotate(-2deg)]"
          ...
          <div
            v-else
            class="flex-shrink-0 w-full sm:w-[140px] h-20 sm:h-[90px] rounded-md border border-[#1a1a1a] bg-[#0e0e0e] flex items-center justify-center text-[28px] font-normal text-[#222]"
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-baseline gap-3 mb-2.5 flex-wrap">
              <component
                :is="project.link && !project.isPrivate ? 'a' : 'h2'"
                :href="project.link && !project.isPrivate ? project.link : undefined"
                :target="project.link && !project.isPrivate ? '_blank' : undefined"
                class="text-base sm:text-lg font-medium text-white tracking-[-0.01em] no-underline transition-colors duration-200"
